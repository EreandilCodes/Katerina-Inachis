...
// GET /api/programming — published posts, optional ?lang=en
router.get('/', async (req, res) => {
  try {
    ...
    const items = await db.prepare(`
      ...
      FROM programming_posts WHERE is_published = 1
      ORDER BY published_at DESC, created_at DESC
    `).all();
    res.json(items);
  } catch (err) {
    logger.fromError('programming_list_error', err);
    res.status(500).json({ error: 'Chyba serveru' });
  }
});

// GET /api/programming/admin/all — admin
router.get('/admin/all', AuthMiddleware.verifyToken, AuthMiddleware.adminOnly, async (req, res) => {
  try {
    const items = await db.prepare(`SELECT * FROM programming_posts ORDER BY created_at DESC`).all();
    res.json(items);
  } catch (err) {
    logger.fromError('programming_admin_list_error', err);
    res.status(500).json({ error: 'Chyba serveru' });
  }
});

// GET /api/programming/featured — is_featured=1, optional ?lang=en
router.get('/featured', async (req, res) => {
  try {
    ...
    const items = await db.prepare(`
      ...
      FROM programming_posts WHERE is_featured = 1 AND is_published = 1
      ORDER BY published_at DESC
    `).all();
    res.json(items);
  } catch (err) {
    logger.fromError('programming_featured_error', err);
    res.status(500).json({ error: 'Chyba serveru' });
  }
});

// GET /api/programming/:slug — public single, optional ?lang=en
router.get('/:slug', async (req, res) => {
  try {
    ...
    const item = await db.prepare(`SELECT ${selectCols} FROM programming_posts WHERE slug = ? AND is_published = 1`).get(req.params.slug);
    if (!item) return res.status(404).json({ error: 'Příspěvek nenalezen' });
    item.tags = await getTagsForContent('programming_post', item.id);
    res.json(item);
  } catch (err) {
    logger.fromError('programming_get_error', err);
    res.status(500).json({ error: 'Chyba serveru' });
  }
});

// POST /api/programming — admin
router.post('/', AuthMiddleware.verifyToken, AuthMiddleware.adminOnly, async (req, res) => {
  try {
    ...
    const result = await db.prepare(`
      INSERT INTO programming_posts (title, slug, excerpt, content, cover_image, is_featured, is_published, published_at, title_en, excerpt_en, content_en)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    const item = await db.prepare('SELECT * FROM programming_posts WHERE id = ?').get(result.lastInsertRowid);
    logger.info('programming_post_created', { id: result.lastInsertRowid, title });
    res.status(201).json({ message: 'Příspěvek vytvořen', item });
  } catch (err) {
    logger.fromError('programming_create_error', err);
    res.status(err.message?.includes('UNIQUE') ? 400 : 500).json({ error: err.message?.includes('UNIQUE') ? 'Slug již existuje' : 'Chyba serveru' });
  }
});

// PUT /api/programming/:id — admin
router.put('/:id', AuthMiddleware.verifyToken, AuthMiddleware.adminOnly, async (req, res) => {
  try {
    ...
    await db.prepare(`
      UPDATE programming_posts SET title=?, slug=?, excerpt=?, content=?, cover_image=?,
    ...
    const item = await db.prepare('SELECT * FROM programming_posts WHERE id = ?').get(req.params.id);
    logger.info('programming_post_updated', { id: req.params.id });
    res.json({ message: 'Příspěvek aktualizován', item });
  } catch (err) {
    logger.fromError('programming_update_error', err);
    res.status(500).json({ error: 'Chyba serveru' });
  }
});

// DELETE /api/programming/:id — admin
router.delete('/:id', AuthMiddleware.verifyToken, AuthMiddleware.adminOnly, async (req, res) => {
  try {
    await deleteContentTags('programming_post', req.params.id);
    await db.prepare('DELETE FROM programming_posts WHERE id = ?').run(req.params.id);
    logger.info('programming_post_deleted', { id: req.params.id });
    res.json({ message: 'Příspěvek smazán' });
  } catch (err) {
    logger.fromError('programming_delete_error', err);
    res.status(500).json({ error: 'Chyba serveru' });
  }
});
