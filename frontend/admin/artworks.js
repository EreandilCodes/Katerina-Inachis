...
export class ArtworksManager {
  ...
  async loadItems() {
    try {
      const response = await fetch('/api/artworks/admin/all', {
  ...
  renderItems() {
    ...
    tbody.innerHTML = this.items.map(a => `
      <tr>
        ...
        <td>${escHtml(a.collection || '—')}</td>
        <td><span class="badge ${a.is_published ? 'badge-published' : 'badge-draft'}">${a.is_published ? 'Publikovaný' : 'Koncept'}</span></td>
        <td>${a.is_featured ? '<span class="badge badge-featured">Vybrané</span>' : '—'}</td>
        <td>${a.sort_order ?? 0}</td>
        <td class="td-actions">
          <button class="btn-admin btn-admin--outline btn-admin--sm" onclick="window.admin.managers.artworks.showModal(${a.id})">Upravit</button>
          <button class="btn-admin btn-admin--danger btn-admin--sm" onclick="window.admin.managers.artworks.deleteItem(${a.id})">Smazat</button>
        </td>
      </tr>`).join('');
  }

  showModal(id = null) {
    const item    = id ? this.items.find(a => a.id === id) : null;
    const overlay = document.getElementById('artworksModalOverlay');
    const titleEl = document.getElementById('artworksModalTitle');
    ...
    form.elements.medium.value       = item?.medium       || '';
  ...
  async saveItem(id) {
    ...
    const body = {
      ...
      medium:      form.elements.medium.value.trim(),
  ...
  closeModal() {
    document.getElementById('artworksModalOverlay')?.classList.add('hidden');
  }
}
