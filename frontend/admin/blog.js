...
export class BlogManager {
  constructor(auth, admin) {
    ...
    this.rte   = null;
    this.rteEn = null;
    this.tagPicker = new TagPicker(admin, 'blogTagsChips', 'blogTagsInput');
  }
  ...
  renderItems() {
    ...
    tbody.innerHTML = this.items.map(b => `
      <tr>
        ...
        <td class="td-actions">
          <button class="btn-admin btn-admin--outline btn-admin--sm" onclick="window.admin.managers.blog.showModal(${b.id})">Upravit</button>
          <button class="btn-admin btn-admin--danger btn-admin--sm" onclick="window.admin.managers.blog.deleteItem(${b.id})">Smazat</button>
        </td>
      </tr>`).join('');
  }
  ...
  showModal(id = null) {
    ...
    if (item) this.tagPicker.loadFor('blog_post', item.id);
  }
  ...
  async saveItem(id) {
    ...
    try {
      ...
      if (savedId) {
        try {
          await this.tagPicker.assign('blog_post', savedId);
        } catch (err) {
  ...
  closeModal() {
    document.getElementById('blogModalOverlay')?.classList.add('hidden');
  }
}
